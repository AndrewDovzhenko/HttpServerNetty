...
public class HttpServerStatistics implements Serializable {
	...
	public List<List<String>> getConnectionsAsStrings() {
		...
		connections.forEach((connectionInfo) -> {
			...
			s.add(connectionInfo.getIp());
			s.add(connectionInfo.getUrisAsString());
			s.add(getFormattedDateTime(connectionInfo.getEstablished()));
			if (connectionInfo.getClosed() != null) {
				s.add("" + connectionInfo.getBytesSent());
				s.add("" + connectionInfo.getBytesReceived());
				s.add(String.format("%.3f", connectionInfo.getSpeed()));
			} else {
				s.add("–");
				s.add("–");
				s.add("–");
	...
	public boolean equals(Object o) {
		...
		if (connections != null ? !connections.equals(that.connections) : that.connections != null)
			return false;
		if (ipRequests != null ? !ipRequests.equals(that.ipRequests) : that.ipRequests != null)
			return false;
